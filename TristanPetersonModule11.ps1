...
Last edit: 2021-11-03
Version 2 - inital release of Test-CloudFlare
...
# Enabling cmdlet binding.
    ...
    "Text" {
        ...
        Add-Content -path './Remnettest.txt' -Value "Date Tested: $DateTime"
        Add-Content -path './Remnettest.txt' -Value "Computer: $Computername"
        Add-Content -path './Remnettest.txt' -Value (Get-Content -path './TestResults.txt')
        Write-Verbose "Finished running test, Displaying"
        notepad.exe './Remnettest.txt'
        remove-item './TestResults.txt'}
        #Generates file and displays it, also removes old TestResults.txt file.
